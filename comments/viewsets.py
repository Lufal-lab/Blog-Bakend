...
@extend_schema_view(
    list=extend_schema(
        ...
        description="Return a paginated list of comments. Can be filtered by post.",
        parameters=[
            OpenApiParameter(
                name="post",
                type=int,
    ...
    destroy=extend_schema(
        ...
        description="Delete a comment. Only the comment author or staff can delete it.",
        responses={
...
class CommentViewSet(ModelViewSet):
    """
    API endpoints for managing comments on blog posts.

    - Anyone can list or retrieve comments
    ...
    def get_queryset(self):
        ...
        Optionally filter by post using the `post` query parameter.
        """
        queryset = Comment.objects.all().order_by("-created_at")
    ...
    def get_serializer_context(self):
        """
        Add the related Post to the serializer context when creating comments.
        Supports nested routes and direct post references.
        """
        context = super().get_serializer_context()
    ...
    def perform_create(self, serializer):
        """
        Save the comment.
        User and post are handled in the serializer.
        """
        serializer.save()
