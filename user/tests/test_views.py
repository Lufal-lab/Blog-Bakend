...
class TestUserViews:
    """
    Test suite for user-related views and API endpoints.
    ...
    def setup_method(self):
        """Setup method to get the CustomUser model and ensure default team exists."""
        self.User = get_user_model()
        Team.objects.get_or_create(name="Default")

    # -----------------------------
    # Unit tests for admin access
    # -----------------------------
    def test_normal_user_cannot_access_admin(self, client):
        """Normal users should be redirected (302) when trying to access the admin panel."""
    ...
    def test_logout_works(self, client):
        """Logout should redirect user when accessing protected endpoints."""
    ...
    def test_superuser_can_access_admin_with_force_login(self, client):
        """Superusers can access the admin panel using force_login without normal login."""
    ...
    def test_session_cookie_creation_and_usage(self, client):
        """
        Test that logging in creates a session cookie and that it can be used
